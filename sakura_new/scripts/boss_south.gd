...
# ============================================================
# BOSS SOUTH — Arena del Goblin Shaman
# Goblin Shaman — Pantano Maldito
# Fases: veneno (fase2), tormenta de hechizos (fase3)
# ============================================================

const ARENA_W: int = 640
const ARENA_H: int = 480

const BOSS_NAME     := "Goblin Shaman"
const BOSS_LEVEL    := 45
const BOSS_MAX_HP   := 4500
const BOSS_MOB_TYPE := "goblin"

const MUSIC_KEY := "world_south"

const C_FLOOR     := Color(0.15, 0.22, 0.12)
const C_WALL      := Color(0.10, 0.16, 0.08)
const C_RUNE      := Color(0.30, 0.90, 0.30)
const C_DOOR_LOCK := Color(0.50, 0.35, 0.10)
const C_DOOR_OPEN := Color(0.25, 0.70, 0.25)
...
func _draw_arena() -> void:
	...
	_add_wall(Vector2(0, -hh),  Vector2(ARENA_W, 32))
	_add_wall(Vector2(0,  hh),  Vector2(ARENA_W, 32))
	_add_wall(Vector2(-hw, 0),  Vector2(32, ARENA_H))
	_add_wall(Vector2( hw, 0),  Vector2(32, ARENA_H))
	var runes = ["ᚷ","ᛟ","ᛒ","ᛚ","ᛁ","ᚾ","ᛋ","ᚨ"]
	for i in runes.size():
		var angle = (i / float(runes.size())) * TAU
		var rpos  = Vector2(cos(angle), sin(angle)) * 150.0
		var lbl   = Label.new()
		lbl.text = runes[i]; lbl.position = rpos; lbl.z_index = 2
		lbl.add_theme_font_size_override("font_size", 18)
		lbl.add_theme_color_override("font_color", C_RUNE)
		add_child(lbl)
		var tw = create_tween().set_loops()
		tw.tween_property(lbl, "modulate:a", 0.2, 1.2 + i * 0.1)
		tw.tween_property(lbl, "modulate:a", 1.0, 1.2 + i * 0.1)
	...
	zone_lbl.add_theme_color_override("font_color", Color(0.5, 1.0, 0.2))
	add_child(zone_lbl)
...
func _close_door() -> void:
	...
	var lbl = Label.new(); lbl.text = "[ CERRADO ]"; lbl.position = Vector2(-35, hh - 45)
	lbl.z_index = 10; lbl.name = "DoorLabel"
...
func _spawn_boss() -> void:
	...
	_spawn_boss_aura(_boss_node, Color(0.30, 0.90, 0.20))
	...
	print("[BossSouth] %s invocado" % BOSS_NAME)
...
func _apply_phase(phase: int) -> void:
	...
	match phase:
		2:
			if "speed"      in _boss_node: _boss_node.speed      = 100.0
			if "attack_dmg" in _boss_node: _boss_node.attack_dmg = 25.0
			_boss_node.scale = Vector2(3.3, 3.3)
			_spawn_boss_aura(_boss_node, Color(0.70, 1.00, 0.10))
			_show_phase_notice("☠ %s — FASE 2: ¡Veneno!" % BOSS_NAME, Color(0.7, 1.0, 0.1))
		3:
			if "speed"      in _boss_node: _boss_node.speed      = 140.0
			if "attack_dmg" in _boss_node: _boss_node.attack_dmg = 40.0
			_boss_node.scale = Vector2(3.6, 3.6)
			_spawn_boss_aura(_boss_node, Color(1.0, 0.5, 0.0))
			_show_phase_notice("☠ %s — FASE 3: ¡Tormenta!" % BOSS_NAME, Color(1.0, 0.5, 0.0))
			if has_node("/root/AudioManager"):
				get_node("/root/AudioManager").play_sfx("boss_roar")

	_mechanic_interval = 9.0 if phase == 1 else (6.5 if phase == 2 else 4.5)
	_summon_goblin_adds(phase)

# ──────────────────────────────────────────────────────────
# MECÁNICAS DE COMBATE — Goblin Shaman
# ──────────────────────────────────────────────────────────

## Invoca goblins menores que deben morir rápido o explotan al expirar.
func _summon_goblin_adds(phase: int) -> void:
	if _mechanics == null or not is_instance_valid(_boss_node):
		return
	var count: int = 2 if phase == 2 else 4
	_show_phase_notice("🟢 ¡Goblins invocados! Mátalos antes de que exploten.", Color(0.6, 1.0, 0.4))
	var adds := _mechanics.spawn_adds(_boss_node.global_position, "goblin", count, int(BOSS_LEVEL * 0.6), 110.0)
	_adds_alive = adds
	for a in adds:
...
func _on_adds_timeout() -> void:
	for a in _adds_alive:
		if is_instance_valid(a):
			_mechanics.telegraph_aoe(a.global_position, 90.0, int(BOSS_LEVEL * 5), 0.8,
				Color(0.5, 1.0, 0.3, 0.4), false)
			a.queue_free()
	_adds_alive.clear()
...
func _run_random_mechanic() -> void:
	if _mechanics == null or not is_instance_valid(_boss_node):
		return
	match randi() % 3:
		0: _mech_poison_cloud()
		1: _mech_toxic_trail()
		2: _mech_acid_rain()

## MECÁNICA 1 — Nube de veneno grande centrada en el boss (alejarse).
func _mech_poison_cloud() -> void:
	_show_phase_notice("☣ %s libera una nube tóxica — ¡Alejaos!" % BOSS_NAME, Color(0.5, 1.0, 0.3))
	var dmg := int(BOSS_LEVEL * 5.5 * (1.0 + (_current_phase - 1) * 0.4))
	_mechanics.telegraph_aoe(_boss_node.global_position, 140.0, dmg, 1.8,
		Color(0.4, 0.9, 0.2, 0.4), false)

## MECÁNICA 2 — Rastro tóxico que sigue al Tank actual; obliga a moverse.
func _mech_toxic_trail() -> void:
	var target := _mechanics.get_top_threat_target()
	if not is_instance_valid(target):
		target = get_tree().get_first_node_in_group("player")
	if not is_instance_valid(target):
		return
	_show_phase_notice("☣ ¡Rastro tóxico! El Tank debe reposicionar al jefe.", Color(0.6, 1.0, 0.4))
	for i in range(4):
		var p: Vector2 = target.global_position + Vector2(randf_range(-40, 40), randf_range(-40, 40))
		var dmg := int(BOSS_LEVEL * 4 * (1.0 + (_current_phase - 1) * 0.35))
		_mechanics.telegraph_aoe(p, 60.0, dmg, 1.3 + i * 0.2, Color(0.5, 1.0, 0.3, 0.35), true)

## MECÁNICA 3 — Lluvia ácida: 3 zonas random que castigan quedarse quieto.
func _mech_acid_rain() -> void:
	_show_phase_notice("☣ Lluvia ácida — ¡no te quedes quieto!", Color(0.6, 1.0, 0.3))
	...
	for i in range(3):
		...
		_mechanics.telegraph_aoe(p, 80.0, dmg, 1.6, Color(0.5, 1.0, 0.25, 0.35), true)
...
func _on_boss_defeated() -> void:
	...
	if has_node("/root/BossNotifManager"):
		get_node("/root/BossNotifManager").register_boss_death("south")
	if has_node("/root/AchievementManager"):
		get_node("/root/AchievementManager").on_boss_killed("south")
	...
	print("[BossSouth] %s derrotado!" % BOSS_NAME)
	if has_node("/root/AudioManager"):
	...
	if has_node("/root/InventoryManager"):
		...
		if inv.has_method("add_item"):
			...
			inv.add_item("crystal_shard",               randi_range(10, 18))
			inv.add_item("weapon_staff_holy_t3",        1)   # drop garantizado épico (Báculo de la Luz Divina)
			inv.add_item("material_boss_south_essence", randi_range(1, 2))  # material épico de boss
			if randf() < 0.25:
				inv.add_item("weapon_staff_holy_t4",    1)   # drop legendario: Báculo del Serafín
	if has_node("/root/GameManager"):
		var gm = get_node("/root/GameManager")
		if gm.has_method("add_experience"): gm.add_experience(4500)
	_open_door()
	_activate_return_portal()

func _setup_exit_portal() -> void:
	var portal_gfx = ColorRect.new(); portal_gfx.name = "PortalGfx"
	portal_gfx.color = Color(0.2, 0.3, 0.2, 0.4); portal_gfx.size = Vector2(60, 40)
	portal_gfx.position = Vector2(-30, ARENA_H / 2 - 55); portal_gfx.z_index = 5; add_child(portal_gfx)
	var lbl = Label.new(); lbl.name = "PortalLabel"; lbl.text = "[ Portal — Vence al Boss ]"
	lbl.position = Vector2(-100, ARENA_H / 2 - 80); lbl.z_index = 10
	lbl.add_theme_font_size_override("font_size", 11)
	lbl.add_theme_color_override("font_color", Color(0.5, 0.6, 0.5)); add_child(lbl)

func _activate_return_portal() -> void:
	if has_node("PortalGfx"): get_node("PortalGfx").color = Color(0.20, 0.80, 0.20, 0.85)
	if has_node("PortalLabel"):
		get_node("PortalLabel").text = "→ Portal al Mundo Sur"
		get_node("PortalLabel").add_theme_color_override("font_color", Color(0.3, 1.0, 0.3))
	...
	portal.body_entered.connect(func(body):
		if body.is_in_group("player"):
			...
			get_tree().change_scene_to_file("res://scenes/world_south.tscn")
	)
