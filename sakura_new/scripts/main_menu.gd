...
#   Godot 4.6.3 Android — Pixel Art MMORPG
#
#   ÍNDICE DE SECCIONES (Ctrl+F para navegar):
...
# Host del servidor de multiplayer en tiempo real (WebSocket).
# Mismo dominio que el backend de Railway; el puerto lo decide
# el servicio multiplayer en Railway (se vuelve público vía 443/wss).
const MP_HOST : String = "servidor-multiplayer-sakura-production.up.railway.app"
...
# Peinados disponibles por género (curlyhair es mixto, longhair solo femenino)
const HAIR_STYLES_MALE   := ["bowlhair","curlyhair","mophair","shorthair","spikeyhair"]
const HAIR_STYLES_FEMALE := ["bowlhair","curlyhair","longhair","mophair","shorthair","spikeyhair"]
const SKIN_PRESETS     := [
...
var _delete_confirm_overlay : Control  = null
var _delete_confirm_field   : LineEdit = null
var _delete_confirm_error   : Label    = null
var _delete_confirm_btn     : Button   = null
var _delete_confirm_lbl     : Label    = null
...
var _screen_forgot      : Node     = null
var _forgot_email_field : LineEdit = null
var _forgot_msg_lbl     : Label    = null
var _forgot_send_btn    : Button   = null
...
func _ready() -> void:
	...
	_build_forgot_screen()
	_setup_login_screen()
	_setup_select_screen()
	_setup_create_screen()
	_style_quit_button()
	# Fondo de video del login/registro — sin audio
	if is_instance_valid(login_video_bg):
		login_video_bg.volume_db = -80.0
		login_video_bg.finished.connect(func():
			login_video_bg.play()
		)
...
func _build_epic_background() -> void:
	...
	for i in range(5):
		...
		root.add_child(line)
	# Borde dorado eliminado — pantalla limpia sin marcos visibles
...
func _setup_title_wow() -> void:
	...
	# TitleLabel es ahora un TextureRect con el logo PNG — sin overrides de fuente
	var sub := $SubtitleLabel as Label
	sub.add_theme_color_override("font_color", Color(0.62, 0.55, 0.90, 0.88))
	# Línea dorada, diamante y ornamentos laterales eliminados — pantalla limpia sin marcos visibles
...
func _apply_panel_style(panel: PanelContainer, is_wide: bool = false) -> void:
	var s := StyleBoxFlat.new()
	# Borde dorado eliminado — panel limpio y moderno sin marcos visibles
	s.bg_color = C_BG_PANEL_DARK
	s.set_border_width_all(0)
	s.set_corner_radius_all(8)
	s.shadow_color = Color(0.0, 0.0, 0.0, 0.60); s.shadow_size = 20
...
func _add_panel_top_ornament(_panel: PanelContainer) -> void:
	pass  # Ornamentos dorados eliminados

func _add_panel_bottom_bar(_panel: PanelContainer) -> void:
	pass  # Barra inferior dorada eliminada
...
func _setup_login_screen() -> void:
	...
	forgot_btn.custom_minimum_size = Vector2(0, 28)
	forgot_btn.layout_mode = 2
	forgot_btn.flat = true
	forgot_btn.text = "¿Olvidaste tu contraseña?"
	forgot_btn.add_theme_font_size_override("font_size", 11)
	forgot_btn.add_theme_color_override("font_color", C_TEXT_DIM)
	forgot_btn.add_theme_color_override("font_hover_color", C_GOLD_MID)
	forgot_btn.add_theme_stylebox_override("normal", StyleBoxEmpty.new())
	forgot_btn.add_theme_stylebox_override("hover", StyleBoxEmpty.new())
	forgot_btn.add_theme_stylebox_override("pressed", StyleBoxEmpty.new())
	forgot_btn.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	forgot_btn.pressed.connect(_on_forgot_password_btn_pressed)
...
func _build_verify_screen() -> void:
	...
	panel_style.bg_color = C_BG_PANEL_DARK; panel_style.set_border_width_all(0)
	panel_style.set_corner_radius_all(8)
	panel_style.shadow_color = Color(0.0, 0.0, 0.0, 0.60); panel_style.shadow_size = 20
...
# ════════ [RECUPERAR] Pantalla de recuperación de contraseña ═══════
func _build_forgot_screen() -> void:
	var canvas := CanvasLayer.new()
	canvas.name = "ScreenForgot"; canvas.layer = 10
	canvas.process_mode = Node.PROCESS_MODE_ALWAYS
	canvas.visible = false; add_child(canvas); _screen_forgot = canvas
	...
	panel.anchor_top = 0.5; panel.anchor_bottom = 0.5
	panel.offset_left = -230.0; panel.offset_right = 230.0
	panel.offset_top = -190.0; panel.offset_bottom = 190.0
	var panel_style := StyleBoxFlat.new()
	panel_style.bg_color = C_BG_PANEL_DARK; panel_style.set_border_width_all(0)
	panel_style.set_corner_radius_all(8)
	panel_style.shadow_color = Color(0.0, 0.0, 0.0, 0.60); panel_style.shadow_size = 20
	...
	var title := Label.new(); title.text = "🔑  RECUPERAR CONTRASEÑA"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_font_size_override("font_size", 18)
	title.add_theme_color_override("font_color", C_GOLD_BRIGHT); vbox.add_child(title)
	var sep := HSeparator.new()
	...
	var info := Label.new()
	info.text = "Escribe el Gmail de tu cuenta.\nTe enviaremos un enlace para crear una nueva contraseña."
	info.add_theme_font_size_override("font_size", 12)
	info.add_theme_color_override("font_color", C_TEXT_LIGHT)
	info.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	info.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; vbox.add_child(info)

	var email_lbl := Label.new(); email_lbl.text = "📧  Correo Gmail"
	email_lbl.add_theme_font_size_override("font_size", 12)
	email_lbl.add_theme_color_override("font_color", C_TEXT_DIM); vbox.add_child(email_lbl)

	_forgot_email_field = LineEdit.new()
	_forgot_email_field.custom_minimum_size = Vector2(0, 40); _forgot_email_field.layout_mode = 2
	_forgot_email_field.placeholder_text = "tucuenta@gmail.com"
	_forgot_email_field.add_theme_font_size_override("font_size", 14)
	vbox.add_child(_forgot_email_field)

	_forgot_msg_lbl = Label.new()
	_forgot_msg_lbl.add_theme_font_size_override("font_size", 11)
	_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
	_forgot_msg_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; vbox.add_child(_forgot_msg_lbl)

	_forgot_send_btn = Button.new()
	_forgot_send_btn.custom_minimum_size = Vector2(0, 46); _forgot_send_btn.layout_mode = 2
	_forgot_send_btn.text = "📧  ENVIAR ENLACE DE RECUPERACIÓN"
	_forgot_send_btn.add_theme_font_size_override("font_size", 13)
	_style_button(_forgot_send_btn, true)
	_forgot_send_btn.pressed.connect(_on_forgot_send_pressed); vbox.add_child(_forgot_send_btn)

	var back_btn2 := Button.new()
	back_btn2.custom_minimum_size = Vector2(0, 36); back_btn2.layout_mode = 2
	back_btn2.text = "← Volver al Login"; back_btn2.add_theme_font_size_override("font_size", 12)
	_style_button(back_btn2)
	back_btn2.pressed.connect(func():
		_screen_forgot.visible = false)
	vbox.add_child(back_btn2)

func _on_forgot_password_btn_pressed() -> void:
	if is_instance_valid(_forgot_email_field): _forgot_email_field.text = login_user_field.text
	if is_instance_valid(_forgot_msg_lbl):
		_forgot_msg_lbl.text = ""
		_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
	if is_instance_valid(_forgot_send_btn): _forgot_send_btn.disabled = false
	if is_instance_valid(_screen_forgot): _screen_forgot.visible = true

func _on_forgot_send_pressed() -> void:
	var gmail := _forgot_email_field.text.strip_edges().to_lower()
	if not gmail.ends_with("@gmail.com") or gmail.length() < 10:
		_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
		_forgot_msg_lbl.text = "⚠ Usa un correo @gmail.com válido."; return

	_forgot_send_btn.disabled = true
	_forgot_msg_lbl.add_theme_color_override("font_color", C_GOLD_MID)
	_forgot_msg_lbl.text = "Enviando..."

	var http := HTTPRequest.new(); add_child(http)
	http.timeout = 20.0
	http.request_completed.connect(func(result, code, _headers, body):
		http.queue_free()
		_forgot_send_btn.disabled = false
		if result != HTTPRequest.RESULT_SUCCESS:
			_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
			_forgot_msg_lbl.text = "⚠ No se pudo conectar al servidor."; return
		var json : Variant = JSON.parse_string(body.get_string_from_utf8())
		if not json is Dictionary or not json.get("ok", false):
			_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
			_forgot_msg_lbl.text = "⚠ %s" % json.get("error", "No se pudo procesar la solicitud."); return
		_forgot_msg_lbl.add_theme_color_override("font_color", C_UNCOMMON)
		_forgot_msg_lbl.text = "✅ Si el correo existe, recibirás un enlace para crear una nueva contraseña. Revisa también spam."
	)
	var err := http.request(
		AUTH_BACKEND + "/forgot-password",
	...
	if err != OK:
		_forgot_send_btn.disabled = false
		_forgot_msg_lbl.add_theme_color_override("font_color", C_RED_DANGER)
		_forgot_msg_lbl.text = "⚠ No se pudo conectar al servidor."; http.queue_free()
...
func _setup_select_screen() -> void:
	...
	_delete_confirm_overlay = _build_delete_confirm_dialog(root); root.add_child(_delete_confirm_overlay)
...
func _show_select() -> void:
	...
	if is_instance_valid(_screen_register): _screen_register.visible = false
	if is_instance_valid(login_video_bg):
		login_video_bg.visible = false; login_video_bg.stop()
...
func _show_create(slot_idx: int) -> void:
	...
	_selected_gender = "male"; _update_preview()
	if is_instance_valid(login_video_bg):
		login_video_bg.visible = false; login_video_bg.stop()
...
# ════════ Diálogo de confirmación de borrado de personaje ══════════
func _build_delete_confirm_dialog(_parent: Control) -> Control:
	var overlay := PanelContainer.new(); overlay.layout_mode = 0
	overlay.set_anchors_preset(Control.PRESET_FULL_RECT)
	overlay.mouse_filter = Control.MOUSE_FILTER_STOP; overlay.visible = false
	var overlay_s := StyleBoxFlat.new(); overlay_s.bg_color = Color(0.0, 0.0, 0.0, 0.75)
	...
	var win := PanelContainer.new(); win.layout_mode = 0
	win.anchor_left = 0.5; win.anchor_right = 0.5; win.anchor_top = 0.5; win.anchor_bottom = 0.5
	win.offset_left = -190; win.offset_right = 190; win.offset_top = -140; win.offset_bottom = 140
	var win_s := StyleBoxFlat.new(); win_s.bg_color = Color(0.030, 0.010, 0.010, 0.98)
	win_s.set_border_width_all(1); win_s.border_color = Color(0.65, 0.20, 0.20, 0.65)
	win_s.set_corner_radius_all(5); win_s.shadow_size = 12; win_s.shadow_color = Color(0,0,0,0.6)
	win_s.content_margin_left = 16; win_s.content_margin_right = 16
	win_s.content_margin_top = 14; win_s.content_margin_bottom = 14
	win.add_theme_stylebox_override("panel", win_s); overlay.add_child(win)

	var vbox := VBoxContainer.new(); vbox.layout_mode = 2
	vbox.add_theme_constant_override("separation", 8); win.add_child(vbox)

	var title := Label.new(); title.text = "⚠  ELIMINAR PERSONAJE"
	title.add_theme_font_size_override("font_size", 14)
	title.add_theme_color_override("font_color", C_RED_DANGER)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; vbox.add_child(title)
	vbox.add_child(_make_separator(true))

	_delete_confirm_lbl = Label.new()
	_delete_confirm_lbl.add_theme_font_size_override("font_size", 11)
	_delete_confirm_lbl.add_theme_color_override("font_color", C_TEXT_LIGHT)
	_delete_confirm_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_delete_confirm_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_delete_confirm_lbl.text = "¿Estás seguro de eliminar a \"%s\"? Esta acción es imposible de recuperar."
	vbox.add_child(_delete_confirm_lbl)
	...
	hint_lbl.text = "Escribe el nombre del personaje para confirmar:"
	vbox.add_child(hint_lbl)

	_delete_confirm_field = LineEdit.new()
	_delete_confirm_field.custom_minimum_size = Vector2(0, 38); _delete_confirm_field.layout_mode = 2
	_delete_confirm_field.placeholder_text = "Nombre del personaje"
	_delete_confirm_field.add_theme_font_size_override("font_size", 13)
	_delete_confirm_field.text_changed.connect(func(_t): _on_delete_confirm_field_changed())
	vbox.add_child(_delete_confirm_field)

	_delete_confirm_error = Label.new()
	_delete_confirm_error.add_theme_font_size_override("font_size", 10)
	_delete_confirm_error.add_theme_color_override("font_color", C_RED_DANGER)
	_delete_confirm_error.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_delete_confirm_error.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	vbox.add_child(_delete_confirm_error)

	vbox.add_child(_make_separator(true))
	var btn_row := HBoxContainer.new()
	btn_row.add_theme_constant_override("separation", 8); vbox.add_child(btn_row)

	var cancel_btn := Button.new()
	cancel_btn.custom_minimum_size = Vector2(0, 38); cancel_btn.layout_mode = 2
	cancel_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	cancel_btn.text = "Cancelar"; _style_button(cancel_btn)
	cancel_btn.add_theme_font_size_override("font_size", 12)
	cancel_btn.pressed.connect(_on_delete_confirm_cancel); btn_row.add_child(cancel_btn)

	_delete_confirm_btn = Button.new()
	_delete_confirm_btn.custom_minimum_size = Vector2(0, 38); _delete_confirm_btn.layout_mode = 2
	_delete_confirm_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_delete_confirm_btn.text = "🗑  Eliminar"; _delete_confirm_btn.disabled = true
	_style_button(_delete_confirm_btn, false, true)
	_delete_confirm_btn.add_theme_font_size_override("font_size", 12)
	_delete_confirm_btn.pressed.connect(_on_delete_confirm_accept); btn_row.add_child(_delete_confirm_btn)

	return overlay

func _on_delete_confirm_field_changed() -> void:
	if _selected_slot < 0 or _selected_slot >= _slots.size() or _slots[_selected_slot] == null:
		return
	var cname : String = _slots[_selected_slot].get("name", "")
	var typed : String = _delete_confirm_field.text
	if typed == cname:
		_delete_confirm_btn.disabled = false; _delete_confirm_error.text = ""
	else:
		_delete_confirm_btn.disabled = true
		if typed != "":
			_delete_confirm_error.text = "⚠ El nombre no coincide."
		else:
			_delete_confirm_error.text = ""

func _on_delete_confirm_cancel() -> void:
	if is_instance_valid(_delete_confirm_overlay): _delete_confirm_overlay.visible = false

func _on_delete_confirm_accept() -> void:
	if _selected_slot < 0 or _selected_slot >= _slots.size() or _slots[_selected_slot] == null:
		_on_delete_confirm_cancel(); return
	_slots[_selected_slot] = null; _save_slots(); _selected_slot = -1
	select_error_lbl.text = "Personaje eliminado."
	_refresh_slot_panels(); _refresh_select_buttons()
	_on_delete_confirm_cancel()
...
func _on_create_slot(idx: int) -> void:
	if not _slot_unlocked[idx]: return
	_show_create(idx)
...
func _on_enter_btn_pressed() -> void:
	...
	# para que otros jugadores nos vean y veamos a otros jugadores.
	if not NetworkManager.is_client and not NetworkManager.is_server:
		NetworkManager.join_server(MP_HOST, MP_PORT)
	_go_to_town()

func _on_delete_btn_pressed() -> void:
	if _selected_slot < 0 or _slots[_selected_slot] == null: return
	var cname : String = _slots[_selected_slot].get("name","?")
	if is_instance_valid(_delete_confirm_lbl):
		_delete_confirm_lbl.text = "¿Estás seguro de eliminar a \"%s\"? Esta acción es imposible de recuperar." % cname
	if is_instance_valid(_delete_confirm_field):
		_delete_confirm_field.text = ""
	if is_instance_valid(_delete_confirm_error):
		_delete_confirm_error.text = ""
	if is_instance_valid(_delete_confirm_btn):
		_delete_confirm_btn.disabled = true
	if is_instance_valid(_delete_confirm_overlay):
		_delete_confirm_overlay.visible = true
		if is_instance_valid(_delete_confirm_field):
			_delete_confirm_field.call_deferred("grab_focus")
...
func _process(delta: float) -> void:
	...
	if is_instance_valid(_title_label):
		var pulse := 0.92 + 0.08 * sin(_anim_timer * 1.8)
		_title_label.modulate = Color(pulse, pulse, pulse, 1.0)
